...
public class UserUploadServiceImpl implements UserUploadService {
	...
	public ApiReturn processUserUpload(MultipartFile fileToUpload, long employeeTypeId, UUID tenantId)
		...
		if (!(employeeTypeId == Constants.COACH || employeeTypeId == Constants.EMPLOYEE
			...
			role = roleRepository.findById(employeeTypeId).get();
		}
		Optional<TenantDetails> tenantDetails = tenantDetailsRepo.findById(tenantId);
		if (!tenantDetails.isPresent()) {
			throw new CommonException("Invalid Tenant Details");
		}

		XSSFSheet spreadsheet = convertCsvToXlxs(fileToUpload).getSheet("sheet1");
		...
		for (int rowNo = 1; rowNo <= spreadsheet.getLastRowNum(); rowNo++) {
			...
			if (currentRow.getCell(2) != null && currentRow.getCell(2).getCellType() == CellType.STRING) {
				...
				try {
					if (userRepo.existsByEmailAndTenantId(email, tenantDetails.get().getId())) {
			...
			if (currentRow.getCell(3) != null && currentRow.getCell(3).getCellType() == CellType.STRING) {
				...
				if (phone.trim().length() != 10) {
					reason = reason != null ? reason + "/ Inavlid Mobile Number" : "Inavlid Mobile Number";
					skipUpload = true;
				}
			} else {
	...
	private XSSFWorkbook convertCsvToXlxs(MultipartFile fileToUpload) throws IOException {
		...
		while ((currentLine = br.readLine()) != null) { // NOPMD by sajid on 6/12/17 12:08 PM
